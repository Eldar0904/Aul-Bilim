...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // about.html — cooperation hub flip cards
    var coopHub = document.getElementById('coop-hub');
    if (coopHub) {
      var coopCards = Array.prototype.slice.call(coopHub.querySelectorAll('.coop-hub-card'));

      function setCoopCardFlipped(card, flipped) {
        if (!card) return;
        var hit = card.querySelector('.coop-hub-card-hit');
        var back = card.querySelector('.coop-hub-face--back');
        card.classList.toggle('is-flipped', flipped);
        if (hit) hit.setAttribute('aria-expanded', flipped ? 'true' : 'false');
        if (back) back.setAttribute('aria-hidden', flipped ? 'false' : 'true');
      }

      function closeCoopCards(except) {
        coopCards.forEach(function (card) {
          if (card !== except) setCoopCardFlipped(card, false);
        });
      }
      ...
      coopCards.forEach(function (card) {
        ...
        hit.addEventListener('click', function (e) {
          ...
          closeCoopCards(flipped ? card : null);
          setCoopCardFlipped(card, !flipped);
        });
      });

      document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape') closeCoopCards();
      });

      document.addEventListener('click', function (e) {
        if (!e.target.closest('.coop-hub-card')) closeCoopCards();
      });
    }
